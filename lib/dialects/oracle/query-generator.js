    ...
    selectQuery: function(tableName, options, model) {
      ...
      if (options.include) {
        var generateJoinQueries = function(include, parentTable) {
          ...
          var concatAttribute = include.concatAttribute || '.';
          if (tableName !== parentTable && mainTableAs !== parentTable) {
            as = parentTable + concatAttribute + include.as;
          }
          ...
          if (through) {
            ...
            , tableRight = as, attrRight = association.associationType !== 'BelongsTo' ?
              association.identifierField || association.identifier :
              association.targetIdentifier || right.rawAttributes[primaryKeysRight[0]].field || primaryKeysRight[0], joinOn, subQueryJoinOn;
